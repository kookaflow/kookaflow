...
export const Route = createFileRoute("/api/public/hooks/send-weekly-reminder")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        for (const p of prefs ?? []) {
          ...
          if (
            ...
            p.weekly_reminder_channel !== "email" &&
          ...
          const { subject, html } = buildWeeklyEmail({
            ...
            tipSeed:
              zoned.year * 53 + Math.floor(zoned.day / 7) + zoned.month,
          });
