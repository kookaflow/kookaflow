...
import { l as logo, u as useServerFn, B as Button, c as cn, d as usePreferences, T as THEMES, g as getPushStatus, f as updatePushPrefs, b as createSsrRpc } from "./router-CPotmhHx.js";
import { T as ThemeToggle } from "./ThemeToggle-fXnfEenI.js";
import { useState, useEffect, useCallback, useRef } from "react";
import { Link, useNavigate } from "@tanstack/react-router";
import { ChevronRight, Sparkles, ExternalLink, Loader2, RotateCcw, Layers, LogOut, CalendarRange, Coffee, CalendarCheck2, AlertTriangle, RefreshCw, Unplug, Check, Sun, Moon, Monitor, Bell, BellOff, Mail, Smartphone, Volume2, Play, CloudRain, Waves, Trees, Download, FileText, FileSpreadsheet, ShieldCheck, HelpCircle, Scale, Star, Share2, AlertOctagon, Trash2 } from "lucide-react";
import { C as Card, a as CardHeader, b as CardTitle, c as CardDescription, d as CardContent } from "./card-acy-qbGY.js";
import { toast } from "sonner";
import { s as supabase } from "./client-BHPGm-jd.js";
import { u as useSubscription } from "./useSubscription-CFnWWwpf.js";
import { a as createCustomerPortalSession } from "./stripe.functions-D1Z7WLsB.js";
import { h as openNativeSubscriptionManagement, r as restoreRevenueCatPurchases, a as refreshRevenueCatEntitlements } from "./revenuecat-D10VleS8.js";
import { S as Switch, g as getGoogleConnectionStatus, d as disconnectGoogleCalendar, t as triggerGoogleSync, a as getGoogleAuthUrl } from "./google-calendar.functions-CuhmOYzv.js";
...
import { L as Label } from "./label-D779wBT-.js";
import { I as Input } from "./input-O2mPIqdU.js";
import OneSignal from "react-onesignal";
import { z } from "zod";
import { r as requireSupabaseAuth } from "./auth-middleware-DPkeKsny.js";
import { e as createServerFn } from "./server-ONNXnefI.js";
import jsPDF from "jspdf";
import { u as useEvents } from "./EventsProvider-BF0MLJv_.js";
import { D as Dialog, a as DialogContent, b as DialogHeader, c as DialogTitle, d as DialogDescription, e as DialogFooter } from "./dialog-DjQH_cn0.js";
import "clsx";
...
function AccountSection() {
  ...
  useServerFn(createCustomerPortalSession);
  ...
  const isAppleSubscriber = !!native && (native.store === "APP_STORE" || native.store === "MAC_APP_STORE");
  const isNativeLifetime = !!native?.isLifetime;
  const cadenceLabel = native?.periodLabel === "yearly" ? "yearly" : native?.periodLabel === "monthly" ? "monthly" : null;
  const nativeEntKey = sub.nativeEntitlements.pro ? "pro" : sub.nativeEntitlements.basic ? "basic" : null;
  const nativePaid = nativeEntKey !== null;
  const tierLabel = nativePaid && isNativeLifetime ? "Lifetime Pro" : nativePaid && nativeEntKey === "pro" ? cadenceLabel === "yearly" ? "Pro Yearly" : cadenceLabel === "monthly" ? "Pro Monthly" : "Pro" : nativePaid && nativeEntKey === "basic" ? cadenceLabel === "monthly" ? "Basic Monthly" : "Basic" : sub.tier === "lifetime" ? "Lifetime Pro" : sub.tier === "pro" ? "Pro" : sub.tier === "basic" ? "Basic" : sub.tier === "expired" ? "Expired" : sub.isTrialing ? "Free trial" : "Trial";
  const tierBadgeClass = nativePaid && (nativeEntKey === "pro" || isNativeLifetime) || !nativePaid && (sub.tier === "pro" || sub.tier === "lifetime") ? "bg-primary/15 text-primary" : nativePaid && nativeEntKey === "basic" || !nativePaid && sub.tier === "basic" ? "bg-accent/30 text-accent-foreground" : !nativePaid && sub.tier === "expired" ? "bg-destructive/15 text-destructive" : "bg-muted text-muted-foreground";
  const baseSubtitle = sub.tier === "lifetime" ? "Lifetime access — thanks for your support" : sub.tier === "pro" ? "Pro — all features unlocked" : sub.tier === "basic" ? "Basic — upgrade for the dashboard & sync" : sub.tier === "expired" ? "Trial ended — upgrade to keep using Pro" : sub.isTrialing ? `${sub.trialDaysRemaining} day${sub.trialDaysRemaining === 1 ? "" : "s"} left in your free trial` : "14-day trial";
  const subSubtitle = nativePaid && isNativeLifetime ? "Lifetime access — thanks for your support" : nativePaid ? "Active" : isNativeLifetime && sub.tier !== "basic" ? "Lifetime access — thanks for your support" : cadenceLabel && (sub.tier === "pro" || sub.tier === "basic") ? `${sub.tier === "pro" ? "Pro" : "Basic"} — ${cadenceLabel}` : baseSubtitle;
  const renewalLine = native && !native.isLifetime && native.expirationDate ? native.willRenew ? `Renews ${formatDate(native.expirationDate)}` : `Access ends ${formatDate(native.expirationDate)}` : null;
  const showAppleManage = isAppleSubscriber && !isNativeLifetime;
  const showStripeManage = false;
  const showUpgrade = nativePaid ? nativeEntKey === "basic" && !isNativeLifetime : sub.tier === "trial" || sub.tier === "basic" || sub.tier === "expired";
    ...
    /* @__PURE__ */ jsxs(Card, { className: "p-0 overflow-hidden", children: [
      ...
      /* @__PURE__ */ jsxs("div", { className: "flex flex-wrap items-center gap-3 p-4", children: [
        ...
        /* @__PURE__ */ jsxs("div", { className: "flex-1 min-w-0", children: [
          ...
          renewalLine && /* @__PURE__ */ jsx("p", { className: "truncate text-xs text-muted-foreground", children: renewalLine })
        ] }),
        /* @__PURE__ */ jsx("span", { className: `rounded-full px-2.5 py-1 text-[11px] font-semibold ${tierBadgeClass}`, children: tierLabel }),
        /* @__PURE__ */ jsxs("div", { className: "flex w-full flex-wrap gap-2 sm:w-auto", children: [
        ...
        /* @__PURE__ */ jsx("div", { className: "w-full", children: /* @__PURE__ */ jsxs(
          Button,
          {
            ...
            children: [
              restoring ? /* @__PURE__ */ jsx(Loader2, { className: "size-4 animate-spin" }) : /* @__PURE__ */ jsx(RotateCcw, { className: "size-4" }),
              "Restore purchases"
            ]
          }
        ) })
