...
import { b as NO_ENTITLEMENTS, o as onRevenueCatEntitlementsChange, c as getRevenueCatEntitlements, d as getRevenueCatSubscriptionInfo } from "./revenuecat-D10VleS8.js";
const DEFAULT_STATE = {
  loading: true,
  signedIn: false,
  tier: "trial",
...
function computeDerived(tier, status, trialEndsAt, subscriptionEndDate, native = NO_ENTITLEMENTS) {
  ...
  const inPaidGracePeriod = (status === "canceled" || status === "past_due") && !!subscriptionEndDate && subscriptionEndDate.getTime() > now;
  const proActive = tier === "pro" && (status === "active" || status === "trialling" || inPaidGracePeriod) || tier === "lifetime";
...
function useSubscription() {
  ...
  const applyNativeSubscription = useCallback((info) => {
    nativeSubRef.current = info;
    setState(
      (prev) => prev.nativeSubscription === info ? prev : { ...prev, nativeSubscription: info }
    );
  }, []);
  useEffect(() => {
    let cancelled = false;
